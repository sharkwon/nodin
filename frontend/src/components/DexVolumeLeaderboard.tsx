...
import type { DexVolumeItem } from "../lib/nodin";
...
export function DexVolumeLeaderboard() {
  const [data, setData] = useState<DexVolumeItem[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch("/api/insight/dex-volume")
      .then((r) => r.json())
      .then((d: DexVolumeItem[]) => {
        setData(d);
        setLoading(false);
      })
  ...
  if (loading) {
    return (
      <div className="border-b border-border px-6 lg:px-10 py-10">
        <h3 className="text-lg font-bold text-foreground mb-1">DEX Volume — 24h</h3>
        <p className="text-xs text-muted-foreground mb-6">Solana DEX trading volume leaderboard</p>
  ...
  return (
    <div className="border-b border-border px-6 lg:px-10 py-10">
      ...
      <p className="text-xs text-muted-foreground mb-6">
        Solana DEX trading volume leaderboard — top {data.length} by 24h volume
      </p>
